import { CommandHandler } from '@nestjs/cqrs';
import { ReviewReportCommand } from './review-report.command';
import { Result } from '@libs/utils';
import { ExceptionBase } from '@libs/base-classes';
import { CommandHandlerBase } from '@libs/base-classes/command-handler.base';
import { ReportsUnitOfWork } from '@modules/reports/database/unit-of-work';
import { ReportEntity } from '@modules/reports/domain';
import { UuidVO } from '@libs/value-objects';

@CommandHandler(ReviewReportCommand)
export class ReviewReportCommandHandler extends CommandHandlerBase<
  ReportsUnitOfWork,
  ReportEntity
> {
  constructor(unitOfWork: ReportsUnitOfWork) {
    super(unitOfWork);
  }

  async handle(
    command: ReviewReportCommand,
  ): Promise<Result<ReportEntity, ExceptionBase>> {
    const {
      trxId,
      payload: { id, reviewerId },
    } = command;

    const repository = this.unitOfWork.getReportRepository(trxId);

    const reportResult = await repository.getOneById(new UuidVO(id));
    const report = reportResult.unwrap();

    report.startReview(new UuidVO(reviewerId));

    return repository.update(report);
  }
}
